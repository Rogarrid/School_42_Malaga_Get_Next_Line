...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/12/01 08:52:45 by rogarrid          #+#    #+#             */
/*   Updated: 2022/12/01 09:21:30 by rogarrid         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (fd < 0 || BUFFER_SIZE <= 0 || fd > 1024)
		return (0);
	save_line[fd] = ft_read_save_next_line(fd, save_line[fd]);
	if (!save_line[fd])
		return (NULL);
	line_return = ft_print_line(save_line[fd]);
	save_line[fd] = ft_clean_save(save_line[fd]);
	return (line_return);
}
